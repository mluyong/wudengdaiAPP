<template>
  <div>
     ...
     <!-- <div class="clearfix order-header text-center">
        <div class="float_left order-header-left">
            <img src="../../../static/img/go-back-left.png" alt="">
        </div>
        <p>我的订单</p>
    </div> -->
    <div class="order-all">
        <ul id="all">
           <li v-for="(order,index) in order" v-bind:key="order.text" v-bind:class="{checked:index==nowIndex}" v-on:click="orderClick(index)">{{order.text}}</li>
        </ul>
    </div>
...
export default {
  data() {
    return {
      order: [{ text: "全部" }, { text: "待付款" }, { text: "进行中" }, { text: "已完成" }],
      nowIndex: -1
    };
  },
  methods: {
